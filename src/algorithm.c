...
/*   Updated: 2024/09/23 20:54:50 by zpiarova         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	find_least_operations(t_stack *a, t_stack *b)
{
	...
	t_operation c_o; // current operation - stores how many operations ra, rb,
	rra, rrb, rr,
		rrr it takes to get two corresponding elements to top of each stack temp = a->head;
	...
	while (temp)
	{
		...
		if (((d_o.ra + d_o.rrb) < c_o.rrcount) && ((d_o.ra
					+ d_o.rrb) < c_o.rrrcount) && ((d_o.ra
					+ d_o.rrb) < a->cheapest_o->count))
		{
			set_operation(a, d_o.ra, 0, 0, d_o.rrb);
			a->cheapest_o->count = d_o.ra + d_o.rrb;
		}
		else if (((d_o.rb + d_o.rra) < c_o.rrcount) && ((d_o.rb
					+ d_o.rra) < c_o.rrrcount) && ((d_o.rb
					+ d_o.rra) < a->cheapest_o->count))
		{
			// a->cheapest_o->ra = 0;
		...
		else if (c_o.rrcount < c_o.rrrcount
			&& c_o.rrcount < a->cheapest_o->count)
		{
		...
		else if (c_o.rrrcount < c_o.rrcount
			&& c_o.rrrcount < a->cheapest_o->count)
		{
			a->cheapest_o->ra = 0;
			a->cheapest_o->rb = 0;
			a->cheapest_o->rr = 0;
		...
		// printf("el: %d, fr: %d, ra: %d, rb: %d, rr: %d, rra: %d.
		// rrb: %d. rrr: %d\n", temp->value, friend->value, a->cheapest_o->ra,
		// a->cheapest_o->rb, a->cheapest_o->rr, a->cheapest_o->rra,
		// a->cheapest_o->rrb, a->cheapest_o->rrr);
		// temp = temp->next;
	}
}
...
// --> now a is sorted in muliple parts,
so just find if firts is not min or
	last is not max
	// if distance a->first and a->min is smaller than half of a size ra,
	else rra void
	back_to_a(t_stack * a, t_stack *b)
{
	t_node	*successor;
	...
	{
		while (a->first != a->min)
			// number of elements that need to go to the end are smaller so we ra
			ra(a);
	}
	else
	{
